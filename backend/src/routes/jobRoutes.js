...
router.post('/', auth, async (req, res) => {
  ...
  try {
    // Check if user is employer (optional - you can skip this check)
    const userCheck = await pool.query(
      'SELECT user_type FROM users WHERE id = $1',
      [req.user.id]
    );
    
    // If you want to restrict job posting to employers only, uncomment this
    // if (userCheck.rows[0].user_type !== 'employer') {
